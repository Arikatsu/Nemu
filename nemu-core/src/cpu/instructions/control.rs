...
pub(in crate::cpu) fn jr_imm8(ctx: &mut InstructionContext) -> u8 {
    ...
    let pc = ctx.cpu.regs.pc().wrapping_add(offset as u16);
    ctx.cpu.regs.set_pc(pc);
    12
}
...
pub(in crate::cpu) fn ret(ctx: &mut InstructionContext) -> u8 {
    let ret_addr = ctx.memory.read_u16(ctx.cpu.regs.sp());
    ctx.cpu.regs.inc_sp(2);
    ctx.cpu.regs.set_pc(ret_addr);
    16
}
...
pub(in crate::cpu) fn rst(ctx: &mut InstructionContext, vec: u8) -> u8 {
    ...
    ctx.memory.write_u16(sp, ret_addr);
    ctx.cpu.regs.set_sp(sp);
    ctx.cpu.regs.set_pc(vec as u16);
    16
}