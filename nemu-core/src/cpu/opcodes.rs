...
pub(super) fn ld_r8_imm8<B: Bus>(cpu: &mut Cpu<B>, reg: Reg8) -> u8 {
    let value = cpu.memory.borrow().read(cpu.regs.pc());
    cpu.regs.inc_pc(1);
...
pub(super) fn ld_r16_imm16<B: Bus>(cpu: &mut Cpu<B>, reg: Reg16) -> u8 {
    let value = cpu.memory.borrow().read_u16(cpu.regs.pc());
    cpu.regs.inc_pc(2);
...
pub(super) fn ld_mem_imm16_sp<B: Bus>(cpu: &mut Cpu<B>) -> u8 {
    let addr = cpu.memory.borrow().read_u16(cpu.regs.pc());
    cpu.regs.inc_pc(2);
    let sp = cpu.regs.sp();
    cpu.memory.borrow_mut().write_u16(addr, sp);
    20
}

/// LDH (imm8), A - Store A at address 0xFF00 + immediate 8-bit value
pub(super) fn ldh_mem_imm8_a<B: Bus>(cpu: &mut Cpu<B>) -> u8 {
    let offset = cpu.memory.borrow().read(cpu.regs.pc());
    cpu.regs.inc_pc(1);
...
pub(super) fn pop_r16<B: Bus>(cpu: &mut Cpu<B>, reg: Reg16) -> u8 {
    let sp = cpu.regs.sp();
    let value = cpu.memory.borrow().read_u16(sp);
    cpu.regs.write_reg16(reg, value);
